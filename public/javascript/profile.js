    ...
    fetch(`/api/users/me`)
        ...
        .then(function (data) {
            for (var i = 0; i < data.posts.length; i++) {
                var dateParts = data.posts[i].created_at.split("-");
                ...
                description.textContent = data.posts[i].content;
                userPost.textContent = data.posts[i].title;
                userNamePost.textContent = data.username;
                ...
                for (var x = 0; x < data.posts[i].comments.length; x++) {
                    userNameComment.textContent = data.posts[i].comments[x].user.username;
                    comments.textContent = data.posts[i].comments[x].comment_text;

                    var dateParts = data.posts[i].comments[x].created_at.split("-");
                    var jsDate = new Date(dateParts[0], dateParts[1] - 1, dateParts[2].substr(0, 2));
                    var month = jsDate.getUTCMonth() + 1;
                    var day = dateParts[2].substr(0, 2);
                    var year = jsDate.getFullYear();
                ...
                postContainer.innerHTML += "<input type='text' id=" + data.posts[i].id + "comment" + " name='comment-text' class=' border-black mb-2 form-control block h-half w-full px-3py-1.5 text-basefont-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none'>" + "<button onclick='addComment()''  id=" + data.posts[i].id + "  type='submit' class='bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full'>" + "Add Comment" + "</button>";
            }
        });
}
getPosts();
